...
export default {
  ...
  <body>
    ...
    try {
      ...
      // Update chat
      if (path.startsWith('/api/chats/') && !path.includes('/typing') && request.method === 'POST') {
      ...
      if (path.match(/^\/api\/chats\/[^/]+\/typing$/) && request.method === 'POST') {
        const chatId = path.split('/')[3];
        const body = await request.json();
        
        const response = await fetch(
          `${FIREBASE_CONFIG.databaseURL}/typing/${chatId}.json`,
          {
            method: 'PUT',
            body: JSON.stringify(body)
          }
      ...
      if (path.match(/^\/api\/chats\/[^/]+\/typing$/) && request.method === 'GET') {
        const chatId = path.split('/')[3];
        
        const response = await fetch(`${FIREBASE_CONFIG.databaseURL}/typing/${chatId}.json`);
        const data = await response.json();
        return new Response(JSON.stringify(data || {}), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        });
      ...
      // Check for stale applications (no chat activity >24hrs) and send reminders
      if (path === '/api/check-stale-applications' && request.method === 'POST') {
        try {
          // Get all applications
          const appsResponse = await fetch(`${FIREBASE_CONFIG.databaseURL}/applications.json`);
          const applications = await appsResponse.json();
          
          // Get all chats
          const chatsResponse = await fetch(`${FIREBASE_CONFIG.databaseURL}/chats.json`);
          const chats = await chatsResponse.json();
          
          // Get all jobs to find assigned employers
          const jobsResponse = await fetch(`${FIREBASE_CONFIG.databaseURL}/jobs.json`);
          ...
          if (applications) {
            for (const [appId, app] of Object.entries(applications)) {
              // Skip if already hired or rejected
              if (app.status === 'hired' || app.status === 'rejected') continue;
              
              const appMessages = chats?.[appId] || [];
              const lastMessageTime = appMessages.length > 0 
                ? Math.max(...appMessages.map(m => m.timestamp || 0))
                : app.appliedDate || 0;
              
              const hoursSinceLastMessage = (now - lastMessageTime) / (1000 * 60 * 60);
              
              // If more than 24 hours since last message
              if (hoursSinceLastMessage > 24) {
                const job = jobs && Object.values(jobs).find(j => j.id === app.jobId);
                const assignedEmployer = job?.assignedEmployer;
                
                if (assignedEmployer) {
                  // Find employer Discord ID
                  const employerEntry = Object.entries(USERS).find(([id, user]) => user.name === assignedEmployer);
                  
                  if (employerEntry) {
                    const [discordId] = employerEntry;
                    ...
                    try {
                      ...
                      if (dmChannel.id) {
                        await fetch(`https://discord.com/api/v10/channels/${dmChannel.id}/messages`, {
                          ...
                          body: JSON.stringify({
                            embeds: [{
                              ...
                              description: `This is a reminder about the application from **${app.data.name || 'Unknown'}** for **${job?.title || 'Unknown Position'}** at **${job?.company || 'Unknown Company'}**.`,
                              color: 0xff9500,
                              fields: [
                                {
                                  name: '⏱ Time Since Last Activity',
                                  value: `${Math.floor(hoursSinceLastMessage)} hours`,
                                  inline: true
                                },
                                {
                                  name: '👤 Candidate',
                                  value: app.data.name || 'Unknown',
                                  inline: true
                                }
                              ],
                              footer: {
                                text: 'Cirkle Careers Portal - Please review this application'
                              },
                        ...
                        reminders.push({
                          appId,
                          candidate: app.data.name,
                          employer: assignedEmployer,
                          hours: Math.floor(hoursSinceLastMessage)
                        });
                      }
                    } catch (dmError) {
                      console.error('Error sending reminder DM:', dmError);
                    }
                  }
                }
              }
          ...
          return new Response(JSON.stringify({ 
            success: true, 
            reminders,
            message: `Sent ${reminders.length} reminder(s)` 
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
          });
        } catch (error) {
